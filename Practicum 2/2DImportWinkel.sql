CREATE OR REPLACE FUNCTION ImportWinkel(  -- IN Varchar2, out Varchar2
    Naam           IN VARCHAR2,
...
BEGIN
  ...
  -- Verbetert alle foute steden
  IF v_Stad          = '''S-Gravenhage' 
  THEN
    v_Stad          := '''s-Gravenhage';
  ELSIF v_Stad       = 'Bergen Op Zoom' 
  THEN
    v_Stad          := 'Bergen op Zoom';
  ELSIF v_Stad       = '''S-Hertogenbosch' 
  THEN
    v_Stad          := '''s-Hertogenbosch';
  ELSIF v_Stad       ='Etten Leur' 
  THEN
    v_Stad          := 'Etten-Leur';
  ELSIF v_Stad       ='De Plaetse' 
  THEN
    v_Stad          :='de Plaetse';
  ELSIF v_Stad       ='Ijmuiden' 
  THEN
    v_Stad          :='IJmuiden';
  ELSIF v_StraatNaam = 'Zevenkampsering,' 
  THEN
    v_StraatNaam    := 'Zevenkampse Ring';
  ELSIF v_Stad       = 'Alphen Aan Den Rijn' 
  THEN
    v_Stad          := 'Alphen aan den Rijn';
  ELSIF v_Straatnaam = 'tVlak' THEN
    v_Straatnaam    := '''t Vlak';
  ELSIF v_Stad       = 'Berkel En Rodenrijs' 
  THEN
    v_Stad          := 'Berkel en Rodenrijs';
  END IF;
  IF v_Stad          = 'HoogvlietRotterdam' 
  THEN
     v_Stad          :='Hoogvliet Rotterdam';
  ELSIF v_Stad       = 'Capelle Aan Den Ijssel' 
  THEN
     v_Stad          :='Capelle aan den IJssel';
  ELSIF v_StraatNaam = 'Van Hogendorpplein' 
  THEN
     v_StraatNaam    := 'van Hogendorpplein';
  ELSIF v_StraatNaam = 'P.C.Hooftlaan' 
  THEN
     v_StraatNaam    := 'P.C. Hooftlaan';
  ELSIF v_StraatNaam = 's-Gravenzandseweg' 
  THEN
      v_StraatNaam    :='''s-Gravenzandseweg';
  ELSIF v_Stad       = 'Oud Beijerland' 
  THEN
      v_Stad          := 'Oud-Beijerland';
  ELSIF v_StraatNaam = 'P S Gerbrandystraat' 
  THEN
      v_StraatNaam    := 'P.S. Gerbrandystraat';
  ELSIF v_StraatNaam = 'GeneraalEisenhouwerplein' 
  THEN
      v_StraatNaam    := 'Generaal Eisenhowerplein';
  ELSIF v_Straatnaam = 'Van Hogendorplaan' 
  THEN
      v_Straatnaam    := 'van Hogendorplaan';
  ELSIF v_Straatnaam = 'van Leeuwenhoeklaan' 
  THEN
...
--Checkt of het niet het huisnummer is met / of -
IF v_HuisnummerMin IS NULL AND v_HuisnummerMax IS NULL THEN
  -- onderscheid de cijfers en letters
  FOR i IN 1..LENGTH(v_Huisnummer)
  LOOP
    v_CheckTeken := SUBSTR(v_Huisnummer,i,1);
...
-- Controleert de postcode huisnummer en straat woonplaats
...
EXCEPTION 
  ...
  raise_application_error(-20000,'de combi klopt niet');
  /*dbms_output.put_line('De waarde van SenW : '|| v_CheckSenW ||'De waarde van PenH  : '|| v_CheckPenH);
  dbms_output.put_line('de combi klopt niet');*/
END;